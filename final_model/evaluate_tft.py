...
def main() -> None:
    ...
    # forward_returns contains realized forward returns aligned to each sample window.
    # Note: this script currently uses the true target returns (y_*_true[:, signal_idx])
    # for trading evaluation; forward_returns are kept here for potential extensions/debugging.
    ...
    if str(getattr(MODEL_CONFIG, "task_type", "quantile_forecast")) == "quantile_forecast":
        ...
        # --- Inference ---
        # Shapes:
        #   y_*_true: (N, H)     true multi-horizon targets
        #   y_*_pred: (N, H, Q)  predicted quantiles for each horizon step
        ...
        for thr in threshold_grid:
            ...
            # Long-only signal rule:
            # - score >= thr means "strong enough risk-adjusted forecast"
            # - mu > 0 prevents going long when the median forecast is negative
        ...
        # --- Slim JSON payload: keep only P0+P1 fields from net backtest dicts ---
        def _prune_backtest_for_json(m: dict) -> dict:
            """
            Keep only decision-critical net metrics in metrics.json.
            Does NOT change computation; only changes what we serialize.
            """
            keep = ("cumulative_return", "sharpe", "max_drawdown", "hit_rate", "avg_trade_return")
            out = {}
            for k in keep:
                v = m.get(k, 0.0)
                out[k] = float(v)  # ensure JSON-friendly (no numpy types)
            return out

        strategy_net_val_out = _prune_backtest_for_json(strategy_net_val)
        strategy_net_test_out = _prune_backtest_for_json(strategy_net_test)
        buy_hold_net_test_out = _prune_backtest_for_json(buy_hold_net_test)
        ...
        metrics_out.update(
            {
                ...
                "costs_and_baselines": {
                    ...
                    "strategy_net": {"val": strategy_net_val_out, "test": strategy_net_test_out},
                    "buy_hold_net": {"test": buy_hold_net_test_out},
                    "random_baseline_summary": {"test": random_baseline_test},
                },
            }
        )
...
def _build_score_mu_iqr(
    ...
    Intuition:
      - mu (median) is the direction/magnitude of expected return.
      - iqr approximates uncertainty (wider band => less confidence).
      - mu / iqr is a simple "confidence-adjusted" return signal.
    """
    if y_pred.ndim != 3:
