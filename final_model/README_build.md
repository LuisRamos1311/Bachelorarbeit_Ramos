# BUILD.md — Final Model (TFT) Build Documentation

This document (“BUILD.md”) describes the **final_model / TFT model** project: a Python pipeline that trains a simplified **Temporal Fusion Transformer–style** model to forecast **24-hour-ahead BTC returns** from **hourly market data**. The model consumes configurable “feature families”: (1) **OHLCV market candles**, (2) **TA-Lib technical indicators** derived from OHLCV, and optionally (3) **daily sentiment** and (4) **daily on-chain** features merged into the hourly timeline in a leakage-aware way. The model produces **quantile forecasts** (q10/q50/q90) to express uncertainty, and the evaluation pipeline converts those forecasts into a **long / cash** trading signal using an **uncertainty-aware score** and a **validation-tuned threshold**.

---

## Experiment Index (compressed)

This index summarizes the evolution that matters for the **final build**. Earlier experiments exist under `finished_experiments/` and were primarily used to converge on the current “quantile forecast + risk-aware thresholding” approach.

| Experiment | Focus | Key change vs prior | Main takeaway (qualitative) | What it enabled |
|---|---|---|---|---|
| 1 | Daily, regression | Multi-horizon return regression | Regression framing did not produce usable signal | Motivated explicit direction framing |
| 2–5 | Daily, classification | 3-class labels + robustness sweeps | Daily direction prediction remained weak; thresholding unstable | Motivated pivot to hourly + more realistic backtest |
| 6 | Hourly baseline | 96h lookback → 24h horizon + non-overlap trades | Hourly framing is more actionable; evaluation realism matters | Established the hourly pipeline constraints |
| 7–7b | Hourly + on-chain | Add daily on-chain; tune threshold for trading objective | Exogenous features can help; objective-aligned thresholding is critical | Trading-oriented selection logic became standard |
| 8 | Add sentiment | Add Reddit + Fear & Greed daily → hourly | Sentiment features extend coverage (2016–2024) and can add signal | Set up the final feature universe |
| 9a–9b | Data integrity + eval | Split-first targets; daily lag; net-of-cost; baselines | Methodology choices materially affect realism | Stabilized the evaluation protocol |
| 9c (final) | Quantile forecasting | Quantile head + uncertainty-aware score | Forecast uncertainty is directly useful for risk filtering | Final architecture + reporting pack |
| 9d (not selected) | Static regime context | Add static regime covariates into TFT | Can become overly conservative depending on calibration | Recorded but not adopted in final build |

Model selection rationale is documented in `final_model/README_model_selection.md`.
...
* `experiments/<eval_id>_score_threshold_sweep.json`
  Full validation sweep across thresholds. This is crucial because thresholding is the main “policy lever” controlling risk and participation.
...
* `plots/<eval_id>_test_forecast_band.png`
  Realized returns vs the predicted quantile band at the signal horizon. Use it to sanity-check uncertainty:
  * does the band widen in volatile regimes?
  * does q50 track the general direction?
