...
def add_target_column(df: pd.DataFrame) -> pd.DataFrame:
    ...
    #
    # Encoding (fixed everywhere in Exp 2):
    #   0 = DOWN  (return < -config.DIRECTION_THRESHOLD)
    #   1 = FLAT  (|return| <= config.DIRECTION_THRESHOLD)
    #   2 = UP    (return >  config.DIRECTION_THRESHOLD)
    thr = config.DIRECTION_THRESHOLD
    # Start with all FLAT
    direction_3c = np.ones(len(df), dtype=np.int64)

    # UP if strictly above +thr
    direction_3c[df["future_return_1d"].values > thr] = 2
    # DOWN if strictly below -thr
    direction_3c[df["future_return_1d"].values < -thr] = 0
